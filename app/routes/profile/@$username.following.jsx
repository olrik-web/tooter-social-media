...
export async function loader({ request, params }) {
  ...
  const following = await db.models.User.find({ _id: { $in: user.following } });
  return json({ following, currentUser });
}

export default function FollowingPage() {
  const { following, currentUser } = useLoaderData();
  ...
  return (
    <div className="flex flex-row">
      <div className="w-full">
        ...
        <h1 className="text-3xl font-bold border border-gray-600 p-4">Following</h1>
        <div className="border-x border-b border-gray-600">
          {following.length === 0 && <p className="text-xl font-bold p-4">Not following anyone yet.</p>}
          {following.map((user) => (
